...
class DraftModeCalenderMonthGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    int getIndexFromDateTime(DateTime d) {
      final months = (d.year - anchorMonth.year) * 12 + (d.month - anchorMonth.month);
      return kAnchorPage + months;
    }

    DateTime getDateTimeFromIndex(int index) {
      final monthsFromAnchor = index - kAnchorPage;
      return DateTime(anchorMonth.year, anchorMonth.month + monthsFromAnchor, 1);
    }

    final PageController pageController = PageController(initialPage: getIndexFromDateTime(dateTime));
    ...
    void onSwipedMonth(int targetIndex) {
      ...
      final maxDay = DraftModeDateTime.getDaysInMonth(monthDateTime.year, monthDateTime.month);
      final sameOrClampedDay = (dateTime.day > maxDay) ? maxDay : dateTime.day;
      final DateTime newDateTime = DateTime(monthDateTime.year, monthDateTime.month, sameOrClampedDay, dateTime.hour, dateTime.minute);
    ...
    List<String> weekdayShort(String locale) {
      try {
        ...
        final list = List.generate(7, (i) => fmt.format(monday.add(Duration(days: i))));
        return list.map((s) => s.characters.take(2).toString().toUpperCase()).toList();
      } catch (_) {
              ...
              return SizedBox(
                ...
                child: Row(
                  children: List.generate(7, (col) {
                    ...
                    final isSelected = day.year == dateTime.year &&
